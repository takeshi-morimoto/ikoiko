...
<div id="pageTop">
	...
	<h1 id="catch" style="background-color:#FF3686;">普通の街コンですよー(´・ω・｀)</h1>
	...
	<div id="mainContainer">
		...
		<div id="mainContent">
			...
			<div id="searchResult">
			    ...
			    $result = $db->query("
			                  select events.find, events.title, events.date, events.week , events.begin , events.end, events.pr_comment, area.page, area.place, events.price_m, events.price_f, area.price_h , area.area , area.area_ja , area.content , events.img_url
			                  from events join area using(area)
			                  where {$where_1} {$where_2} and area.page = 'machi' 
			    ...
			    print 
			        "
			        <h2>{$searchArea}の街コン情報</h2>
			        <div id='resultList'>
			        ";

			    while ( $row = $result->fetch() ):

					list( $find, $title, $date, $week, $begin, $end, $pr_comment, $page, $place, $price_m, $price_f, $price_h, $area, $area_ja, $content, $img_url ) = $row ;
